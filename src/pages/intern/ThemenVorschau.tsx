...
/**
 * Internal preview of the thematic overview (formerly the public Home page).
 * Unlinked and lazy-loaded: the underlying M:N assignment is a draft and not
 * authorised by the city, so it must not appear in the citizen-facing app.
 * Kept intact so the feature can be demonstrated and reviewed as-is.
 */
export default function ThemenVorschau() {
  usePageTitle("Themen-Vorschau (intern)");
  ...
  const onEvents = useMemo(
    () => ({
      click: (p: unknown) => {
        ...
        if (d && !d.children && d.id) navigate(`/intern/themen/${d.id}`);
      },
    }),
    [navigate],
  );
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-lg border border-ink-line bg-white p-4">
        ...
        {!tree.length ? (
          ...
          <PackedCircles data={tree} height={560} onThemeClick={(id) => navigate(`/intern/themen/${id}`)} />
        ) : viz === "sunburst" ? (
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <ul className="lg:w-56 shrink-0 grid grid-cols-2 lg:grid-cols-1 gap-x-4 gap-y-1.5 self-center text-sm">
              {legend.map((l) => (
                <li key={l.id}>
                  <button
                    onClick={() => navigate(`/intern/themen/${l.id}`)}
